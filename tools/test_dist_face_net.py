...
def main():
    ...
    BACKBONE_PATH = os.path.join(cfg.OUTPUT_DIR, 'BACKBONE')
    BACKBONE_ACC = {}
    for BACKBONE in os.listdir(BACKBONE_PATH):
        ...
        for TEST_PATH in TEST_PATH_LIST:
            ...
            # logger.info("{} the {:s} rank-1 program consume time {:.4f} seconds and the accuracy is {:.4f}".format(BACKBONE,TEST_PATH,total_time, acc))
            TESTS_ACC[os.path.basename(TEST_PATH)] = acc
        BACKBONE_ACC[BACKBONE]=TESTS_ACC
    for BACKBONE in BACKBONE_ACC:
        for TEST_PATH in BACKBONE_ACC[BACKBONE]:
            acc = BACKBONE_ACC[BACKBONE][TEST_PATH]
            logger.info("{} the {:s} rank-1 program consume time {:.4f} "
                        "seconds and the accuracy is {:.4f}".format(BACKBONE,TEST_PATH,total_time,acc))
    print(BACKBONE_ACC)
