...
from .scatter_gather import scatter_kwargs as scatter_kwargs_chunk
...
class DistributedDataParallel(Module):
    ...
    def __setstate__(self, state):
        ...
        self.process_group = _get_default_group()
        super(FaceDistributedDataParallel, self).__setstate__(state)
    ...
    def forward(self, *inputs, **kwargs):
        ...
        if self.device_ids:
            ...
            if len(self.device_ids) == 1:
                ...
                # self.gather(outputs[0], self.output_device[1])
                output = [self.gather(outputs,output_device) for output_device in self.output_device][0]
...
class FaceDistributedDataParallel(DistributedDataParallel):
    def __init__(self, module, device_ids=None,
                 output_device=None, dim=0, broadcast_buffers=True,
                 process_group=None, bucket_cap_mb=25,
                 find_unused_parameters=False,
                 check_reduction=False,chunk_sizes=None):
        super(FaceDistributedDataParallel,self).__init__(module=module, device_ids=device_ids,
    ...
    def scatter_chunk(self, inputs, kwargs, device_ids):
        return scatter_kwargs_chunk(inputs, kwargs, device_ids, dim=self.dim, chunk_sizes=self.chunk_sizes)

    def forward(self, *inputs,grad_sync=True, **kwargs):
        if self.require_forward_param_sync:
            self._sync_params()

        if self.device_ids:
            if self.chunk_sizes == None:
                inputs, kwargs = self.scatter(inputs, kwargs, self.device_ids)
            else:
                inputs, kwargs = self.scatter_chunk(inputs, kwargs, self.device_ids)
            if len(self.device_ids) == 1:
                output = self.module(*inputs[0], **kwargs[0])
                output = [output]
            else:
                outputs = self.parallel_apply(self._module_copies[:len(inputs)], inputs, kwargs)
                # output  = []
                # for output_device in self.output_device:
                #     output_ = [output.to(output_device) for output in outputs]
                #     output.append(torch.cat(output_) )
                if self.output_device==[]:
                    output = outputs
                else:
                    output = [self.gather(outputs,output_device) for output_device in self.output_device]
                    # output = self.gather(outputs, self.output_device)
        else:
            output = self.module(*inputs, **kwargs)

        if torch.is_grad_enabled() and self.require_backward_grad_sync and grad_sync:
            self.require_forward_param_sync = True
            # We'll return the output object verbatim since it is a freeform
            # object. We need to find any tensors in this object, though,
            # because we need to figure out which parameters were used during
