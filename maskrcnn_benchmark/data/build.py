...
from .collate_batch import BatchCollator, BBoxAugCollator,BUILD_BatchCollator
...
def make_face_data_loader(cfg, is_train=True, is_distributed=False, start_iter=0,transforms=None):
    num_gpus = get_world_size()
    if is_train:
        images_per_batch = cfg.SOLVER.IMS_PER_BATCH
        assert (
    ...
    for dataset in datasets:
        ...
        collator = BUILD_BatchCollator(cfg)
        num_workers = cfg.DATALOADER.NUM_WORKERS
        data_loader = torch.utils.data.DataLoader(
            dataset,
            num_workers=num_workers,
